		...
		function play(){
			...
			for(var i=1;i<=times;i++){	
				...
				setTimeout(function(){
					...
					countDown((resttime-1)/1000);
				},restmoment);
				console.log(index+"休息延时："+restmoment);
				var finishmoment = focustime+resttime+periodtime*(i-1);
				setTimeout(function(){
					if(index==times){
						isPaused = true;
						...
						countDown((waittime-1)/1000);
					}
									
				},finishmoment);
				console.log(index+"结束延时："+finishmoment);
				var beginmoment = periodtime*(i-1);
				setTimeout(function(){
					start.play();
					info.innerText="正在进行第"+index+"个番茄钟，请保持专注！！";
					countDown((focustime-1)/1000);
				},beginmoment);
				console.log(index+"开始延时："+beginmoment);
